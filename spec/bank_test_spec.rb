...
describe BankTest do
  ...
  context '#print_statement' do
    it 'stores the date the transaction was made' do
      @bank.deposit(2)
      expect(@bank.print_statement).to eq [{ balance: 2.0, credit: 2.0, date: @date.to_s, debit: 0.0 }]
    end

    it 'prints the bank statement with date, credit and balance' do
      @bank.deposit(5)
      expect(@bank.print_statement).to eq [{ balance: 5.0, credit: 5.0, date: @date.to_s, debit: 0.0 }]
    end
